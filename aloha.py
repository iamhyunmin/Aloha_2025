...
def display_revue_report(llm_output_text):
    ...
    # 🧭 마케팅 경로 탐색
    # ===================================================================
    # “유지 경로(Keep Line)”가 나오는 경우도 대응
    data['Enhance_line'] = extract_section(r'강화 경로 \(Enhance Line\):\s*(.*?)\s*-\s*(?:보수|유지) 경로', llm_output_text)
    data['Fix_line'] = extract_section(r'(?:보수|유지) 경로 \(Fix Line\):\s*(.*?)\s*-\s*전환 경로', llm_output_text)
    # “최종” 또는 “추천” 문구 모두 허용
    data['Shift_line'] = extract_section(r'전환 경로 \(Shift Line\):\s*(.*?)\s*===== 🏁(?:최종|추천)? 경로', llm_output_text)
    ...
    st.markdown(
        f"<p style='font-size:1.4rem; font-weight:700;'>🚦 우리 가게 신호: {data['traffic_light']}</p>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<p style='font-size:1.2rem; font-weight:700;'> 전략명: {data['strategy_name']}</p>",
        unsafe_allow_html=True,
    )
