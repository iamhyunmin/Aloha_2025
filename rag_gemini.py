...
- 매출금액, 거래건수, 평균 객단가, 순고객수, 매출 순위 비율 값이 50% 이상일 경우  
  “100% - 값”으로 환산하여 순위를 표현하고, 이를 ‘하위 ○%’로 기술하세요. (예: 80% → 하위 20%)
  50% 미만일 경우에는 그대로 ‘상위 ○%’로 표현합니다. (예: 10% → 상위 10%)
  ※ 이때 ‘상위 10%’는 ‘매우 높은 성과’를 의미합니다.

- 결제취소율은 역방향 지표이므로 해석을 반대로 합니다.  
  50% 이상일 경우 “100% - 값”으로 환산 후 ‘상위 ○%’로 표현하고,  
  50% 미만일 경우 ‘하위 ○%’로 표현하세요.  
  (예: 80% → 상위 20%, 20% → 하위 20%)
  ※ 이때 ‘하위 20% 결제취소율’은 ‘매우 낮은 취소율(우수한 상태)’을 의미합니다.

- 중요한 점:  
  “상위/하위 ○%”는 단순히 **순위 표현**일 뿐, 실제 값이 작거나 크다는 뜻이 아닙니다.  

※ 환산 규칙은 '순위형 지표'(예: 매출금액, 거래건수 등)에만 적용합니다.  
...
===== 📍 현재 위치 파악 =====
- 🚦 신호등 등급(🔴, 🟡, 🟢)을 사용하여 전반적인 매장 상태를 직관적으로 표시하세요.
- 잘 가고 있는 구간에는 강점
...
- 아래 항목을 반드시 포함합니다:
  • 전략명 (창의적 이름 가능, 예: ‘새물길 전략’, ‘Value-Up Combo’)  
  • 핵심 아이디어 (2문장 이내)  
  • 채택 근거 (핵심 지표 기반, 구체적 수치 언급 가능)

===== 🧩 운행 안내 =====
- “실행 방법”에는 실제로 사장님이 바로 적용할 수 있는 행동을 구체적으로 제시하세요.
- “기대효과”에는 실행으로 예상되는 변화(지표, 고객 반응, 매출 등)를 수치나 체감 효과 중심으로 작성하세요.
- 예시는 아래와 같은 형식으로 작성하세요.

<실행 방법 예시>
1. **‘시그니처 세트’ 출시:** 인기 메뉴에 디저트를 결합한 세트를 출시하고 매장 내 포스터를 비치합니다.  
...
def generate_revue_answer(user_query, mct_list=None):
    ...
    # 1.5️⃣ 주소 자동 감지 및 필터링 (rag_text 내부에서 추출)
    addr_pattern = r"((서울(?:특별시)?\s*)?(성동구\s*)?[가-힣A-Za-z0-9]+(로|길|대로)\s*\d*)"
    addr_match = re.search(addr_pattern, context_text)  # rag_text 전체에서 감지

    if addr_match:
        addr_filter = addr_match.group(0).strip()

        # rag_text 내부에서 [ADDR= ...] 부분 추출
        ctx_df["ADDR_EXTRACT"] = ctx_df["rag_text"].str.extract(r"\[ADDR=([^\]]+)\]")[0]

        # 필터링 수행
        ctx_df = ctx_df[ctx_df["ADDR_EXTRACT"].str.contains(addr_filter, na=False)]
...
"""
    ...
    #print(closure_hints[:500])  # 500자까지만 미리보기
